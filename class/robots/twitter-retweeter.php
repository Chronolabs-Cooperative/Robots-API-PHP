...
class APIRobotTwitterRetweeter extends APIRobots {
    
    var $_keywords = array();
    
    var $_resource = 'retweeting.diz';
    
    var $_twitter  = NULL;
    
    var $_method   = 'GET';
    
    var $_url      = 'https://api.twitter.com/1.1/statuses/retweet/%s.json';
            ...
            if (strlen($keywd['keyword']) && $actioned == false)
            {
                ...
                foreach($search['statuses'] as $status)
                {
                    if (!$this->logIdentity($status['id']))
                        ...
                        {
                            $getfields = array('id'=>$status['id']);
                            $postfields = array();
                            if(count($init = json_decode($this->_twitter->setGetfield(http_build_query($getfields))->buildOauth(sprintf($this->_url, $status['id']), $this->_method)->performRequest(), true))>0)
                            {
                                
                                if (!checkForErrorResponse($init, 'twitter'))
                                {
                                    
                                    $logid = $this->setLog($this->_robots[$this->_folder]['id'], $this->_plugin['service-id'], $this->_plugin['id'], $status['user']['screen_name'], 'retweet', htmlspecialchars($status['text']), $status['id']);                            
                                    $sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('keywords') . "` SET `total` = `total` + 1, `left` = `left` - 1, `last-id` = '".$status['id']."', `log-id` = '".$logid."' WHERE `id` = "  . $keywd['id'];
                                    if (!$GLOBALS['APIDB']->queryF($sql))
                                        die('SQL Failed '.$sql);
                                    $actioned = time();
                                }
