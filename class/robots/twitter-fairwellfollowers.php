...
class APIRobotTwitterFairwellfollowers extends APIRobots {
    
    var $_responses = array();
    
    var $_resource = 'fairwells.diz';
    ...
    {
        $ids = array();
        if (parent::execute($function, $term, $response))
        {
            ...
            if ($robot = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF($sql)))
            {
                $fairwelluser = array();
                $attempts = 0;
                while(count($fairwelluser)==0||$attempts<20)
                {
                    $sql = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('friends') . "` WHERE `username-id` = '".$robot['id'] . "' AND `fairwell` = '0' AND `followed` < UNIX_TIMESTAMP() ORDER BY RAND()";
                    if ($fairwell = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF($sql)))
                    {
                        $attempts++;
                        $sql = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('usernames') . "` WHERE `typal` = 'follower' AND `id` = '" . $fairwell['friend-id'] . "' AND `mining` > 0";
                        if ($fairwelluser = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF($sql)))
                        {
                            if (strlen($fairwelluser['username'])==0)
                                $fairwelluser = array();
                            else
                                continue;
                        }
                ...
                if (isset($fairwelluser['username']) && strlen($fairwelluser['username'])>0)
                {
                    $this->_twitter = new TwitterAPIExchange($this->_config);
                    $getfields = array('status'=>$response = sprintf($this->_response[mt_rand(0, count($this->_response) - 1)], $fairwelluser['username']), 'auto_populate_reply_metadata' => true);
                    $postfields = array();
                    if(count($init = json_decode($this->_twitter->buildOauth($this->_url, "POST")->setPostfields($getfields)->performRequest(), true))>0)
                    {
                        if (strlen($init['id'])>0)
                        {
                            $logid = $this->setLog($this->_robots[$this->_folder]['id'], $this->_plugin['service-id'], $this->_plugin['id'], $status['user']['screen_name'], 'fairwell', "Reponse: ".$response, $status['id']);
                            $sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('usernames') . "` SET `left` = `left` - 1, `last-id` = '".$status['id']."', `log-id` = '" . $logid . "' WHERE `id` = " . $robot['id'];
                            if (!$GLOBALS['APIDB']->queryF($sql))
                                die('SQL Failed '.$sql);
                            $sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('friends') . "` SET `fairwell` = UNIX_TIMESTAMP(), `last-id` = '".$status['id']."', `log-id` = '" . $logid . "' WHERE `id` = " . $fairwell['id'];
                            if (!$GLOBALS['APIDB']->queryF($sql))
                                die('SQL Failed '.$sql);
